...
import { listarAutosBanco} from '../../../services/fiscalizacaoService';

function AutoBancoListPage() {
  ...
  useEffect(() => {
    const carregarAutos = async () => {
      ...
      try {
        console.log(`🔄 Carregando autos da página ${currentPage}...`);
        
        const responseData = await listarAutosBanco(currentPage, filters);
        ...
        setError(err.message || 'Erro ao carregar autos de banco.');
  ...
  const handleDebug = async () => {
    ...
    try {
      const result = await listarAutosBanco(1, {});
    ...
    return (
      <div className="flex justify-center items-center p-8 min-h-64 bg-gray-50 dark:bg-[#0c0f12] transition-colors duration-300">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="text-lg text-gray-600 dark:text-gray-300 transition-colors duration-300">Carregando autos de banco...</div>
    ...
    return (
      <div className="p-8 bg-gray-50 dark:bg-[#0c0f12] transition-colors duration-300">
        <div className="bg-yellow-100 dark:bg-yellow-900/20 border border-yellow-400 dark:border-yellow-800 text-yellow-700 dark:text-yellow-200 px-6 py-4 rounded-lg transition-colors duration-300">
          ...
          <p className="mt-2">Não foi possível carregar os dados dos autos de banco.</p>
          <button
            onClick={handleDebug}
  ...
  return (
    <div className="p-8 bg-gray-50 dark:bg-[#0c0f12] transition-colors duration-300">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white transition-colors duration-300">Autos de Constatação - Bancos</h1>
      ...
      <div className="mb-6 p-4 bg-gray-50 dark:bg-[#1a1d21] rounded-lg border border-gray-200 dark:border-gray-700 transition-colors duration-300">
        <div className="flex flex-wrap gap-4 items-center">
          ...
          <div className="flex items-center gap-2">
            <label className="text-sm font-medium text-gray-700">Com irregularidades:</label>
            <select
      ...
      {autos.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-lg shadow border-2 border-dashed border-gray-300">
          ...
          <h3 className="text-xl font-medium text-gray-900 mb-2">Nenhum auto de banco encontrado</h3>
          <p className="text-gray-600 mb-6">
            {Object.keys(filters).length > 0 
              ? 'Nenhum auto corresponde aos filtros aplicados.' 
              : 'Ainda não há autos de banco cadastrados.'
            }
          </p>
          <Link
            to="/fiscalizacao/bancos/novo"
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="text-left p-4 font-semibold text-gray-900">Razão Social</th>
                  <th className="text-left p-4 font-semibold text-gray-900">Município</th>
              ...
              <tbody>
                {autos.map((auto) => (
                  <tr key={auto.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    <td className="p-4">
                      <code className="bg-gray-100 px-2 py-1 rounded text-sm font-mono">
                        {auto.numero || `AUTO-${auto.id}`}
                      </code>
                    </td>
                    <td className="p-4">
                      <div className="font-medium text-gray-900">{auto.razao_social || 'N/A'}</div>
                      {auto.nome_fantasia && (
                        <div className="text-sm text-gray-500">{auto.nome_fantasia}</div>
                      )}
                    </td>
                    <td className="p-4 text-gray-700">{auto.municipio || 'N/A'}</td>
                    ...
                    <td className="p-4">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        auto.tem_irregularidades 
                          ? 'bg-red-100 text-red-800 border border-red-200' 
                          : 'bg-green-100 text-green-800 border border-green-200'
                      }`}>
                        {auto.tem_irregularidades ? '⚠️ Com Irregularidades' : '✅ Conforme'}
                      </span>
                    </td>
                    <td className="p-4">
                      <div className="flex gap-2">
                        <Link
                          to={`/fiscalizacao/bancos/${auto.id}`}
                          className="text-blue-600 hover:text-blue-800 font-medium text-sm transition-colors"
                        >
                          👁️ Ver
                        </Link>
                        <Link
                          to={`/fiscalizacao/bancos/${auto.id}/editar`}
...
export default AutoBancoListPage;