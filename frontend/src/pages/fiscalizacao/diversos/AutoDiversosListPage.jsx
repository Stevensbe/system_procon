...
import { listarAutosDiversos} from '../../../services/fiscalizacaoService';

function AutoDiversosListPage() {
  ...
  useEffect(() => {
    const carregarAutos = async () => {
      ...
      try {
        console.log(`🔄 Carregando autos diversos da página ${currentPage}...`);
        
        const responseData = await listarAutosDiversos(currentPage, filters);
        ...
        setError(err.message || 'Erro ao carregar autos diversos.');
  ...
  const handleDebug = async () => {
    ...
    try {
      const result = await listarAutosDiversos(1, {});
  ...
  const contarIrregularidades = (auto) => {
    const irregularidades = [
      ...
      auto.precos_eletronico_fora_padrao,
      auto.ausencia_precos_eletronico,
      auto.fracionados_fora_padrao,
      auto.ausencia_desconto_visibilidade,
      auto.ausencia_exemplar_cdc,
      auto.substituicao_troco
    ];
    ...
    return (
      <div className="flex justify-center items-center p-8 min-h-64 bg-gray-50 dark:bg-[#0c0f12] transition-colors duration-300">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="text-lg text-gray-600 dark:text-gray-300 transition-colors duration-300">Carregando autos diversos...</div>
    ...
    return (
      <div className="p-8 bg-gray-50 dark:bg-[#0c0f12] transition-colors duration-300">
        <div className="bg-yellow-100 dark:bg-yellow-900/20 border border-yellow-400 dark:border-yellow-800 text-yellow-700 dark:text-yellow-200 px-6 py-4 rounded-lg transition-colors duration-300">
          ...
          <p className="mt-2">Não foi possível carregar os dados dos autos diversos.</p>
          <button
            onClick={handleDebug}
  ...
  return (
    <div className="p-8 bg-gray-50 dark:bg-[#0c0f12] transition-colors duration-300">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white transition-colors duration-300">Autos de Constatação - Legislação Diversas</h1>
      ...
      <div className="mb-6 p-4 bg-gray-50 dark:bg-[#1a1d21] rounded-lg border border-gray-200 dark:border-gray-700 transition-colors duration-300">
        <div className="flex flex-wrap gap-4 items-center">
          ...
          <div className="flex items-center gap-2">
            <label className="text-sm font-medium text-gray-700">Advertência:</label>
            <select
              className="px-3 py-1 border border-gray-300 rounded text-sm"
              onChange={(e) => {
                const newFilters = { ...filters };
                if (e.target.value !== '') {
                  newFilters.aplicar_advertencia = e.target.value;
                } else {
                  delete newFilters.aplicar_advertencia;
                }
                setFilters(newFilters);
              }}
            >
      ...
      {autos.length === 0 ? (
        <div className="text-center py-12 bg-white dark:bg-[#1a1d21] rounded-lg shadow border-2 border-dashed border-gray-300 dark:border-gray-600 transition-colors duration-300">
          ...
          <h3 className="text-xl font-medium text-gray-900 dark:text-white mb-2 transition-colors duration-300">Nenhum auto diversos encontrado</h3>
          <p className="text-gray-600 dark:text-gray-300 mb-6 transition-colors duration-300">
            {Object.keys(filters).length > 0 
              ? 'Nenhum auto corresponde aos filtros aplicados.' 
              : 'Ainda não há autos diversos cadastrados.'
            }
          </p>
          <Link
            to="/fiscalizacao/diversos/novo"
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                  ...
                  <th className="text-left p-4 font-semibold text-gray-900 dark:text-white transition-colors duration-300">Advertência</th>
...
export default AutoDiversosListPage;